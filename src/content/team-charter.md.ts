...
- **The backlog is kept in priority order — read it top-down** — The PO **manually ranks the backlog so the top item is the highest priority** (a deliberate vertical ranking, not creation order, not by number). The list is sorted ascending by each item's \`order\` field, so *lowest \`order\` = top of the list = highest priority*. At planning, **always read the open backlog sorted by \`order\` ascending** and look at the top first — that is how the PO sees it. The next release usually starts from the items at the top, and the PO's ordering is the opening proposal for *what's next* — always open to dialogue, never a silent default. When proposing a slice, start from the top items and explain any reason to deviate. ⚠️ Do **not** infer priority from the item number, creation date, or status alone — only the \`order\` field carries the PO's ranking. (Added 2026-06-24 — PO now ranks the backlog by priority.)
- **Cluster related items with a Label** — Open backlog items group into **themes** (e.g. *Invitations*, *Calendar export*, *Training log*, *Profile*, *Goals*). We **persist each item's theme in its \`Label\` field** (the \`tag\` field — single value per item) so the clusters are durable and **filterable in-app via the backlog's Label dropdown**, not just held in a planning note. At planning, identify the theme the top-priority items belong to and pull that cluster together — a release is usually one theme's thinnest end-to-end slice, not scattered items. When new items are created or re-prioritised, set their Label to the right cluster. The canonical cluster list is mirrored in the roadmap note for cross-session reuse. (Added 2026-06-24 — labels repurposed from generic tags to theme clusters.)
...
- **Refine every item before implementing it** — No item is implemented straight from a one-line title. Each selected item is **refined to the Definition of Ready first** — restate the goal and acceptance criteria, discuss open questions, identify affected files, flag risks, state explicit CRUD scope, and write the "How to verify" checklist — and only then, **after the PO's explicit written "go,"** does implementation start. Refinement can happen item-by-item just before each one is built (not necessarily all up front), but it always precedes the code. If refinement reveals an item is really several stories (e.g. a hidden infrastructure dependency), split it and re-rank. (Added 2026-06-24 — made explicit at PO's request.)
...
- **Update the release history table** — Add the release's row (number, outcome, period) to the release-history table in this charter. (Added after 1.13 review — 1.12's row had been missed)
