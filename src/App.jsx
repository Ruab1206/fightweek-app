...
// Stamdata Katalog (Global Skabelon)
...
const LoginScreen = ({ onLogin, onRedirectLogin, error, loading }) => (
    <div className="min-h-screen bg-slate-950 flex flex-col items-center justify-center p-4 text-center">
        <div className="bg-slate-900 p-8 rounded-2xl border border-slate-800 shadow-2xl max-w-sm w-full relative overflow-hidden">
            ...
            <p className="text-slate-400 mb-8 text-sm leading-relaxed">Log ind for at se din plan og koordinere med teamet.</p>
            {error && <div className="bg-red-900/50 border border-red-800 rounded-lg p-3 mb-6 text-xs text-red-200">{error}</div>}
            <div className="space-y-3">
                <button onClick={onLogin} disabled={loading} className="w-full bg-white text-slate-900 font-bold py-3.5 px-4 rounded-xl hover:bg-slate-100 transition-all flex items-center justify-center gap-2 active:scale-95 disabled:opacity-50">
                    <img src="https://www.gstatic.com/firebasejs/ui/2.0.0/images/auth/google.svg" className="w-5 h-5" alt="G" />
                    {loading ? 'Logger ind...' : 'Log ind med Google'}
                </button>
                <button onClick={onRedirectLogin} className="text-slate-500 text-xs hover:text-blue-400 transition-colors flex items-center justify-center gap-1 w-full pt-2">
                    <ExternalLink className="w-3 h-3" /> Virker knappen ikke? Prøv Redirect
                </button>
            </div>
        </div>
    </div>
        ...
        <div className="bg-slate-950 text-slate-200 min-h-screen pb-24 font-sans selection:bg-blue-500/30">
            ...
            <header className="bg-slate-900 p-4 border-b border-slate-800 sticky top-0 z-50 flex justify-between items-center shadow-lg">
                ...
                <div className="flex items-center gap-2">
                    ...
                    <button onClick={() => signOut(auth)} className="p-2 text-slate-600 hover:text-white transition-colors"><LogOut className="w-5 h-5"/></button>
                </div>
            </header>
            ...
            {modalOpen && (
                <SessionModal 
                    ...
                    onDelete={async (id) => {
                        if (!confirm("Slet dette pas?")) return;
                        const newData = JSON.parse(JSON.stringify(scheduleData));
                        ...
                        <div className="space-y-5">
                            <div>
                                ...
                                <div className="flex flex-wrap gap-2">
                                    {CATEGORIES.map(c => (
                                        <button key={c.label} onClick={() => setForm({...form, category: c.label})} className={`px-3 py-1.5 rounded-lg text-[10px] font-black uppercase border transition-all ${form.category === c.label ? `${c.color} text-white border-transparent shadow-lg scale-105` : 'bg-slate-800 border-slate-700 text-slate-500'}`}>{c.label}</button>
                                    ))}
                                </div>
                            </div>
                            <div>
