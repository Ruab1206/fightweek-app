...
4. **Substantially implemented for two proof sources (decision §27).** A shared `CalendarItemDetail`/`CalendarItemCapabilities` read/detail contract now exists, with a source adapter for legacy self-posted training (`81bf1a5`) and a second source adapter for events (`1d98454`). The event adapter deliberately does not project the event's native signup status (`interested`/`signed-up`/`declined`) into the shared contract: those values target distinct future concepts (`CalendarEntry` for `signed-up`, `Favorite` for `interested`) that are documented but not yet implemented, and `declined` has no approved durable target (decision §27). Source-neutrality is proven at the pure contract/adapter level only, across legacy self-posted training and events; no presentation component consumes the contract yet, and full-calendar UI evaluation remains gated until a non-self-posted presentation consumes the contract without redefining domain meaning.
